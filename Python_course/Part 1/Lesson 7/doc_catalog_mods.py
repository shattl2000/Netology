...
def all_name():
    for number_of_documents in documents:
        try:
            print("Номер документа", number_of_documents["number"], "Принадлежит", number_of_documents["name"])
        except KeyError:
            print("У документа", number_of_documents["number"], "нет хозяина")


def main():
    while True:
        print("Введите команду: ", "\n""p,", "l,", "s,", "a,", "d,", "m,", "as,", "all или", "q - выход")
        user_input = input()
        if user_input == 'p':
            people_name()
        elif user_input == 'l':
